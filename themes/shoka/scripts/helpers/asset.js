...
hexo.extend.helper.register("_vendor_js", () => {
  ...
  let result = vendorJs
    ? `<script src="//cdn.jsdelivr.net/combine/${vendorJs}"></script>`
    : "";

  return vendorJs
    ? htmlTag("script", { src: `//cdn.jsdelivr.net/combine/${vendorJs}` }, "")
    : "";
});
  ...
  return urls
    .map((url) =>
      htmlTag("link", {
        ...
        href: url_for.call(
          this,
          `${statics}${css}/${url}?v=${theme_env["version"]}`,
        ),
      }),
    )
    .join("");
    ...
    .map((url) =>
      htmlTag(
        "script",
        {
          src: url_for.call(
            this,
            `${statics}${js}/${url}?v=${theme_env["version"]}`,
          ),
        },
        "",
      ),
