...
const mediaPlayer = function (t, config) {
    ...
    create: function () {
      ...
      this.el = t.createChild(
        "div",
        {
          ...
          innerHTML:
            (t.player.options.type == "audio"
              ? '<div class="preview"></div>'
              : "") +
            '<div class="controller"></div><div class="playlist"></div>',
        },
        "after",
      );
    ...
    create: function () {
      ...
      this.data.map(function (item, index) {
        ...
        if (!tab) {
          ...
          if (t.player.group) {
            tab
              .attr("data-title", t.player.options.rawList[item.group]["title"])
          ...
          onclick: function (event) {
            ...
            if (playlist.index === index && progress.el) {
              if (source.paused) {
                ...
                t.player.seek(
                  source.duration * progress.percent(event, current),
                );
              }
              return;
            }
  ...
  var lyrics = {
    ...
    update: function (currentTime) {
      ...
      if (
        ...
        currentTime < this.data[this.index][0] ||
        !this.data[this.index + 1] ||
        currentTime >= this.data[this.index + 1][0]
      ) {
        for (var i = 0; i < this.data.length; i++) {
          if (
            currentTime >= this.data[i][0] &&
            (!this.data[i + 1] || currentTime < this.data[i + 1][0])
          ) {
            this.index = i;
            var y = -(this.index - 1);
  ...
  var preview = {
    ...
    create: function () {
      ...
      this.el.innerHTML =
        '<div class="cover"><div class="disc"><img src="' +
        current.cover +
        '" class="blur" /></div></div>' +
      ...
      this.el
        .child(".cover")
        .addEventListener("click", t.player.options.events["play-pause"]);
  ...
  var progress = {
    ...
    percent: function (e, el) {
      var percentage =
        ((e.clientX || e.changedTouches[0].clientX) - el.left()) / el.width();
  ...
  var controller = {
    ...
    events: {
      mode: function (e) {
        ...
        controller.btns["mode"].className =
          "mode " + t.player.options.mode + " btn";
    ...
    update: function (percent) {
      controller.btns["volume"].className =
        "volume " + (!source.muted && percent > 0 ? "on" : "off") + " btn";
